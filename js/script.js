...
$(document).ready(function(){
  ...
  var land_url = "";
  var role;
  if(localStorage.role == "" || localStorage.role == null || typeof localStorage.role == "undefined")
  {
    role = "";
  }
  else
  {
    role = localStorage.role.toLowerCase();
    if(role == "departmentuser")
    {
      desg = "System Administrator";
      land_url = "department-user.html";
      $("ul.left_nav li").hide();
      $("ul.left_nav li[rel='adm_only']").show();
    }
    else if(role == "businessuser")
    {
      desg = "System Administrator";
      land_url = "business-user.html";
      $("ul.left_nav li").hide();
      $("ul.left_nav li[rel='adm_only']").show();
    }
    ...
    {
      desg = "System Administrator";
      land_url = "index-oneview-admin.html";
      $("ul.left_nav li").hide();
      $("ul.left_nav li[rel='adm_only']").show();
    }
    else if(role == "designer")
    {
      desg = "Business Analytics Designer & System Administrator";
      land_url = "index-oneview.html";
      //$("ul.left_nav li[rel='adm_only']").hide();
    }
    else if(role == "analyst")
    {
      desg = "Advanced Analyst";
      land_url = "index-oneview-analyst.html";
      $("ul.left_nav li[rel='adm_only']").hide();
    }
    else if(role == "user")
    {
      desg = "Business Analytics User";
      land_url = "index-oneview-user.html";
      $("ul.left_nav li[rel='adm_only']").hide();
    }
    else
    {
  ...
  $("ul.left_nav li[rel='all']").show();
  $("ul.left_nav li[rel='adm_only noti']").show();
  $("#desg").text(desg);
  $("#link_land").attr("href", land_url);
  $("#link_pam_land").attr("href", land_url);
  $(".logo").attr("href", land_url);
  ...
  $("#msg_data_save").hide();
  $("#kpi_map_save_msg").hide();
  
  $("#kpi_thresh_tree").hide();
  $("#thresh_det").hide();
  $("#kpi_det").hide();
  $("#thresh_target").hide();
  $("#kpi_thresh_save_msg").hide();
  $("#thresh_actions").hide();
  //$("#it_sys1").hide();
  //$(".sys_common").hide();
  ...
  $("#ext_list2").hide();
  $("#ext_list3").hide();
  $("#step_actions_scen").hide();
  $("#step_actions_scen2").hide();
  $("#step_actions_ins").hide();
  $("#step_actions_ins2").hide();
  $("#ldd_actions").hide();
  $("#sel_ext_list").hide();
  $("#lbl_it").hide();
  $("#lbl_ins_it").hide();
  ...
  $("#btn_conn_save").click(function(){
    $("#conn_succ_msg2").hide();
    $("#conn_succ_msg3").show();
    setTimeout(function() {
      $('#modal_est_con').modal('hide');
    }, 1000);
  });
  ...
  $("[name='data_scenario']").change(function(){
    if($(this).val() != "")
    {  $("#data_scen_it").show();  }
    else
    {  $("#data_scen_it").hide();  }
  ...
  $("[name='data_insight']").change(function(){
    if($(this).val() != "")
    {  $("#data_ins_it").show();  }
    else
    {  $("#data_ins_it").hide();  }
  ...
  $(".dd_scen_grid").click(function(){
    $(".dd_scen_grid").removeClass("selected");
    var selected_scen = $(this).attr("rel");
    $(this).addClass("selected");
    $("#selected_scen").text(selected_scen).show();
    $("#sel_scen_it").removeAttr("disabled");
  });
  
  $("#sel_scen_it").click(function(){
    if(! ($(this).attr("disabled") == "disabled"))
    {
      $("#scen_data").hide();
      $("#lbl_it").show();
      $("#data_scen_it").show();
      $("#step_actions_scen").hide();
      $("#step_actions_scen2").show();
    }
  });
  
  $(".dd_ins_grid").click(function(){
    $(".dd_ins_grid").removeClass("selected");
    var selected_scen = $(this).attr("rel");
    $(this).addClass("selected");
    $("#selected_ins").text(selected_scen).show();
    $("#sel_ins_it").removeAttr("disabled");
  });
  
  var selected_it = "";
  $(".dd_it_grid").click(function(){
    if(! $(this).hasClass("selected"))
    {
      if(selected_it == "")
      {
        selected_it += $(this).attr("rel");
      }
      else
      {
        selected_it += ", " + $(this).attr("rel");
    ...
    $(this).addClass("selected");
    $("#selected_it_sys").text(selected_it).show();
    $("#sel_scen_it2").removeAttr("disabled");
  });
  
  $("#sel_scen_it_back1").click(function(){
    $("#step_actions_scen2").hide();
    $("#lbl_it").hide();
    $("#data_scen_it").hide();
    $("#step_actions_scen").show();
  ...
  $("#sel_ins_it").click(function(){
    if(! ($(this).attr("disabled") == "disabled"))
    {
      ...
      $("#data_ins_it").show();
      $("#step_actions_ins").hide();
      $("#step_actions_ins2").show();
    }
  });
  
  $("#sel_ins_it_back1").click(function(){
    $("#step_actions_ins2").hide();
    $("#lbl_ins_it").hide();
    $("#data_ins_it").hide();
    $("#step_actions_ins").show();
  ...
  var selected_ins_it = "";
  $(".dd_ins_it_grid").click(function(){
    if(! $(this).hasClass("selected"))
    {
      if(selected_ins_it == "")
      {
        selected_ins_it += $(this).attr("rel");
      }
      else
      {
        selected_ins_it += ", " + $(this).attr("rel");
    ...
    $(this).addClass("selected");
    $("#selected_ins_it_sys").text(selected_ins_it).show();
    $("#sel_ins_it2").removeAttr("disabled");
  });
  
  $("#sel_ins_it2").click(function(){
    if(! ($(this).attr("disabled") == "disabled"))
    {
      $("#data_ins_it").hide();
      $("#step_actions_ins2").hide();
  ...
  var selected_ent_it = "";
  $(".dd_ent_it_grid").click(function(){
    if(! $(this).hasClass("selected"))
    {
      if(selected_ent_it == "")
      {
        selected_ent_it += $(this).attr("rel");
      }
      else
      {
        selected_ent_it += ", " + $(this).attr("rel");
    ...
    $(this).addClass("selected");
    $("#selected_ent_it_sys").text(selected_ent_it).show();
    $("#sel_ent_it2").removeAttr("disabled");
  });
  
  $("#sel_ent_it2").click(function(){
    if(! ($(this).attr("disabled") == "disabled"))
    {
      $("#data_ent_it").hide();
      $("#step_actions_ent2").hide();
      $("#ext_list").show();
      $("#ldd_actions").show();
    }
  });
  
  $("#data_scenario").change(function(){
    if($(this).val() != "")
    {  $("#data_scen_it").show();  }
    else
    {  $("#data_scen_it").hide();  }
  ...
  $("[name='data_system_ent']").change(function(){
    var data_ent_val = $("[name='data_system_ent']:checked");
    ...
    for(var cnt = 0; cnt < data_ent_val.length; cnt++)
    {
      var tmp_val = $(data_ent_val[cnt]).val();
      if(tmp_val == "SAP PM")
      {  tbl1 = true;  }
      else if(tmp_val == "IBM Maximo")
      {  tbl2 = true;  }
    }
  ...
  $("#ext_exec_all").click(function(){
    if(! ($(this).attr("disabled") == "disabled"))
    {
      ...
      $("[name='data_type']").attr("disabled","disabled");
    }
    else
    {  return false;  }
  });
  ...
  $("[name='sys_type']").change(function(){
    if($(this).val() != "IT System")
    {  $("#it_sys1").hide();  }
    else
    {  $("#it_sys1").show();  }
    $(".sys_common").show();
  ...
  $("[name='sys_type2']").change(function(){
    if($(this).val() != "IT System")
    {  $("#it_sys2").hide();  }
    else
    {  $("#it_sys2").show();  }
  ...
  $("[name='atm_sys_type2']").change(function(){
    if($(this).val() != "IT System")
    {  $("#atm_it_sys2").hide();  }
    else
    {  $("#atm_it_sys2").show();  }
  ...
  $("[name='dtl_tag_type']").change(function(){
    if($(this).val() != "IT System")
    {  $("#dtl_it_sys2").hide();  }
    else
    {  $("#dtl_it_sys2").show();  }
  ...
  $(".ud_kpi_info").click(function(){
    $("#kpi_create").hide();
    // Reset the row styling
    $("#ud_kpi_list").children("tbody").children(".info").removeClass("info");
    
    var parent_row = $(this).parents("tr");
    parent_row.addClass("info");
    var name_value = parent_row.children().eq(0).text();
    $("#asset_udkpi_sel").text(name_value);
    $("#asset_udkpi_info").show();
  });
...
//  $("#cr_kpi_name, [name='cr_kpi_insight'], [name='cr_kpi_scen']").change(function(){
  $("#cr_kpi_name, #cr_kpi_insight, #cr_kpi_scen").change(function(){
    var new_kpi_name = $("#cr_kpi_name").val();
    var sel_kpi_ins = $("#cr_kpi_insight").val();
    var sel_kpi_scen = $("#cr_kpi_scen").val();
  ...
  $("#btn_kpi_save").click(function(){
    ...
    if(! ($(this).attr("disabled") == "disabled"))
    {
      ...
      var new_kpi_ins = sel_kpi_ins.join(", ");
      var new_kpi_scen = sel_kpi_scen.join(", ");
  ...
  $("#rep_hier").change(function(){
    ...
    else if($(this).val() == "Ristin Reporting Hierarchy")
    {
      $("#loc_1").show();
      $("#loc_2").hide();
    }
  ...
  $("#kpi_thresh_save").click(function(){
    $("#kpi_thresh_save_msg").show();
  ...
  $("#loc_1").click(function(){
    ...
    $("#collapseOne").hide();
    $("#atm_loc_arw").toggleClass("fa-chevron-up");
  ...
  $("#btn_add_tag").click(function(){
    if(! ($(this).attr("disabled") == "disabled"))
    {
      ...
      else
      {
        ...
        var td_data = $("#tbl_asset_tag_map [rel='" + sel_asset + "']");
        var link_td_data = $("#tbl_asset_tag_map [rel='" + sel_asset + "']").prev();
  ...
  $("#atm_sys_rep_hier").change(function(){
    if($(this).val() == "Ristin Reporting Hierarchy")
    {
      $("#sel_rep_hier").show();
    }
  });
  ...
  $("#btn_atm_sys_next4").click(function(){
    if(! ($(this).attr("disabled") == "disabled"))
    {
      ...
      var display_state = $("#atm_sys_create5 .tag_limit").is(":hidden");
      console.log(display_state);
      
      if(display_state == true)
      {  $("#atm_sys_create5 .tag_limit").show();  }
    }
  });
  ...
  $("#rep_gen_ok").click(function(){
    //Fetch the values entered in the "Add report details" table and append the same to listing table
    var arr_rep_name = $("[name^='rep_name_']");
    var arr_rep_desc = $("[name^='rep_desc_']");
    var cnt=0;
    var rep_tbl_markup = "";
    
    if(arr_rep_name.length > 0)
    {
      $.each(arr_rep_name, function(){
        rep_tbl_markup += "<tr class='success'>";
        rep_tbl_markup += "<td class='text-center'>&nbsp;</td>";
        rep_tbl_markup += "<td>" + $(this).val() + "</td>";
        rep_tbl_markup += "<td>" + arr_rep_desc.eq(cnt).val() + "</td>";
        rep_tbl_markup += "<td class='text-center'><a href='http://roltaoneview.rolta.com:81/BOE/OpenDocument/opendoc/openDocument.jsp?user=administrator&sIDType=CUID&iDocID=AUA3_vXgzCZBu8o_wD4R.x0' target='_blank' title='View Dashboard' class='fa fa-eye'></a></td>";
        rep_tbl_markup += "<td class='text-center'><a href='javascript:;' title='Edit' class='fa fa-edit'></a></td>";
        rep_tbl_markup += "<td class='text-center'><a href='javascript:;' title='Edit' class='fa fa-trash'></a></td>";
        rep_tbl_markup += "</tr>";
    ...
    $("#tbl_rep_list tbody").prepend(rep_tbl_markup);
    reset_sr("#tbl_rep_list tbody");
  ...
  $("#btn_login").click(function(){
    ...
    if(user_name == "departmentuser")
    {
      redirect_to = "department-user.html";
      user_desg = "System Administrator";
    }
    else if(user_name == "businessuser")
    {
      redirect_to = "business-user.html";
      user_desg = "System Administrator";
    }
    else if(user_name == "auditor")
    {
      redirect_to = "auditor.html";
      user_desg = "System Administrator";
    }
    ...
    else if(user_name == "administrator")
    {
    ...
    else if(user_name == "user")
    {
      ...
      user_desg = "Business Analytics User";
    }
    else if(user_name == "analyst")
    {
      redirect_to = "index-oneview-analyst.html";
      user_desg = "Advanced Analyst";
    }
    else if(user_name == "designer")
    {
      redirect_to = "index-oneview.html";
      user_desg = "Business Analytics Designer";
    }
    else
    {
      alert("Invalid Username! Please enter 'departmentuser', 'businessuser', 'auditor', or 'admin'.");
      //alert("Invalid Username! Please enter 'Administrator', 'Analyst', 'Designer', or 'User'.");
      return false;
    }
    localStorage.setItem("role", user_name);
    localStorage.setItem("desg", user_desg);
    
    $("#frm_login").attr("action", redirect_to);
    $("#frm_login").submit();
  });
  ...
  $(".link_more").click(function(){
    $("#db_coll").slideToggle();
    var more_txt = $(this).html();
    
    if(more_txt == 'View more <span class="fa fa-chevron-down"></span>')
    {  $(this).html('Collapse <span class="fa fa-chevron-up"></span>');  }
    else
    {  $(this).html('View more <span class="fa fa-chevron-down"></span>');  }
  });
  
  $(".link_more_sec").click(function(){
  ...
  /* Code for initializing pagination for table */
  var pag_tbl_elm = $(".tbl_pg").length;
  if(pag_tbl_elm > 0)
  {
    $(".tbl_pg").DataTable({
      "ordering": false,
...
function reset_sr(tbl_obj, sr_column)
{
  var tbl_rows = $(tbl_obj + " tr");
  var rec_no = 1;
  $.each(tbl_rows, function(){
    $(this).children().eq(0).text(rec_no);
    rec_no++;
  });
}
...
$(window).bind("load", function(){
  ...
  function resize_iframe()
  {
    ...
    var ht_reduce = $(window).height() - ($(".btn_bar").height() + 45) - (headHeight+footerHeight+76);
    $("iframe").css("height", ht_reduce);
  }
  
  setInterval(resize_iframe,200);
  ...
  function positionFooter(){
    ...
    if(($("#content-wrapper-dashboard").height()+headHeight+footerHeight) < $(window).height()){
      $footer.css({
        position: "absolute",
        top: footerTop
      })
  ...
  /* function for dynamically setting the height of menu panel to eliminate white space */
  function setMenuPanelHt()
  {
    footerHeight = $footer.height();
    var $sideMenu = $(".menuWrap");
    var body_ht = $("html").height();
    var windowHt = $(window).height();
    var leftMenuHt = 0;
    
    if(windowHt > body_ht)
    {
      leftMenuHt = windowHt - footerHeight - headHeight - 29;
    }
    else
    {
      leftMenuHt = body_ht - footerHeight - headHeight - 29;
    }
    $sideMenu.css("height",leftMenuHt + "px");
  }
  
  setInterval(setMenuPanelHt,200);
});
